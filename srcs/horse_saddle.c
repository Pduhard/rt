	...
	t_3vecf horse_saddle_origin, t_leq l)
{
	...
	a = param->x_fact;
	b = param->y_fact;
	cst.val[2] = (o[0] * o[0]) / (a * a) - (o[2] * o[2]) / (b * b) - o[1];//c
	cst.val[1] = (2 * o[0] * d[0]) / (a * a) - (2 * o[2] * d[2]) / (b * b) - d[1];//b
	cst.val[0] = (d[0] * d[0]) / (a * a) - (d[2] * d[2]) / (b * b);//a
	return (cst);
}

int check_horse_saddle_bnd(t_leq l, t_3vecf horse_saddle_origin, double root, t_horse_saddle *param)
{
	double	x;
	double	y;
	double	z;
