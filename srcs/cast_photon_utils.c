...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/05/18 01:01:35 by aplat             #+#    #+#             */
/*   Updated: 2020/05/18 01:02:34 by aplat            ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
void		absorb_photon(t_leq l, t_phtn_cast p, t_photon photon)
{
	if (p.depth == PHOTON_DEPTH)
		return ;
	photon.color.val[0] = p.pwr.val[0] / (p.photon_type == 1 ?
		(double)NB_INDIRECT_PHOTON : (double)NB_CAUSTIC_PHOTON);
	photon.color.val[1] = p.pwr.val[1] / (p.photon_type == 1 ?
		(double)NB_INDIRECT_PHOTON : (double)NB_CAUSTIC_PHOTON);
	photon.color.val[2] = p.pwr.val[2] / (p.photon_type == 1 ?
		(double)NB_INDIRECT_PHOTON : (double)NB_CAUSTIC_PHOTON);
	if (photon.color.val[0] != photon.color.val[0] || photon.color.val[1]
		!= photon.color.val[1] || photon.color.val[2] != photon.color.val[2])
		exit(0);
	photon.direction = assign_3vecf(-l.dir.val[0], -l.dir.val[1],
		-l.dir.val[2]);
